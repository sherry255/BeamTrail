-module(bt_approval_deadline_workflow).
...
first_decisive_input(View, TimerId) ->
    ...
    TimerDecisions =
        case maps:get(TimerId, maps:get(timers, View, #{}), undefined) of
            #{status := fired, fired_event_seq := Seq} = Timer when is_integer(Seq) ->
                [{Seq, timeout, Timer}];
            _ ->
                []
        end,
...
notify(Input) ->
    case maps:get(test_pid, Input, undefined) of
        undefined -> ok;
        TestPid -> TestPid ! {approval_workflow_executed, fulfill, Input}
    end.
