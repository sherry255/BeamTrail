-module(bt_retry_heartbeat_backoff_workflow).
...
retry_policy(_StepId) ->
    #{max_attempts => 2, backoff_ms => 200, retryable_errors => [transient]}.
...
execute(StepId, _StepVersion, Input, Ctx) ->
    ...
    maps:get(test_pid, Input) !
        {retry_backoff_execute, Attempt, maps:get(idempotency_key, Ctx)},
    case Attempt of
        1 -> {error, transient};
        _ -> {ok, #{step => StepId}}
    end.
